...
const Config = require('../../../../buildconfig/baseConfig.js').subapp.react;
...
// Bundler 选项
const options = {
  outDir: Config.dist, // 将生成的文件放入输出目录下，默认为 dist
  outFile: Config.FileName || 'index.html', // 输出文件的名称
  publicUrl: Config.publicUrl || './', // 静态资源的 url ，默认为 '/'
  watch: false, // 是否需要监听文件并在发生改变时重新编译它们，默认为 process.env.NODE_ENV !== 'production'
  cache: true, // 启用或禁用缓存，默认为 true
  cacheDir: '.cache', // 存放缓存的目录，默认为 .cache
  contentHash: false, // 禁止文件名hash
  global: Config.global || 'reactapp', // 在当前名字模块以UMD模式导出，默认禁止。
